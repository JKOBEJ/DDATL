...
resources_dir = os.path.join(tests_dir, 'xml_resources')
antlr = os.getenv('ANTLR')


@pytest.mark.parametrize('test, inp, exp, grammar, rule, input_format', [
    ('test-json-obj-arr-foo', 'inp-obj-xml.xmi', 'exp-obj-xml-Jim.xmi', 'XMLLexer.g4 XMLParser.g4', 'xml', None),
...
def test_cli(test, inp, exp, grammar, rule, input_format, args, tmpdir):
    ...
    with open(os.path.join(out_dir, inp)) as outf:
        outb = outf.read()
        print(outb)

    with open(os.path.join(resources_dir, exp)) as expf:
        expb = expf.read()
        print(expb)
    assert outb == expb